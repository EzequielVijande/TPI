...
FILTERS_DICT = {"Plano3x3": f.constant_kernel(3), "Plano5x5": f.constant_kernel(5),
                ...
                "Sobel N": f.directional_kernel(f.dir_dict["N"]), "Sobel NE": f.directional_kernel(f.dir_dict["NE"]),
                "Sobel SE": f.directional_kernel(f.dir_dict["SE"]), "Sobel S": f.directional_kernel(f.dir_dict["S"]),
                "Sobel SW":f.directional_kernel(f.dir_dict["SW"]), "Sobel W":f.directional_kernel(f.dir_dict["W"]),
                "Sobel NW":f.directional_kernel(f.dir_dict["NW"]),}
...
def view_histogram(master, img_index):
    if not showing_hist[img_index]:
        ...
        buf = io.BytesIO()
        buf.seek(0)
        fig = plt.figure(figsize=(IMAGE_SIZE/my_dpi,
                         IMAGE_SIZE/my_dpi), dpi=my_dpi)
...
def save_img(img):
    if img is not None:
        filename = filedialog.asksaveasfilename(
            filetypes=IMAGE_TYPES, defaultextension=IMAGE_TYPES)
        img.save(filename)
...
def filter_img(img, master):
    ...
    img_arr[-1] = Image.fromarray(f.conv2d(kernel,
                                  img).astype(np.uint8), mode="L")
...
def create_image_frame(master):
    ...
    operations_menu = OptionMenu(
        second_image_frame, selected_filter, *FILTERS_DICT.keys())
